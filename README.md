...
一键部署会自动准备 D1（`DB` → `myna-edge`）。Token **可选**，Deploy 页不会强制填写；需要鉴权时部署后再到 Dashboard 添加即可。
...
1. 优先使用上方 **Deploy to Cloudflare**（自动建 D1；token 非必填）
2. 或用 Dashboard **Connect GitHub**（详见文档）
